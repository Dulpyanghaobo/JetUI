...
public struct JetSimpleSettingsView: View {
    ...
    @Environment(\.jetSettingsActionHandler) private var settingsActionHandler
            ...
            List {
                Section(header: sectionHeader) {
                    ...
                    settingRow(
                        ...
                        Task { await performSettingsAction(.restorePurchases) }
                    }
                    ...
                    settingRow(
                        ...
                        Task { await performSettingsAction(shareAction) }
                    }
                    ...
                    settingRow(
                        ...
                        Task { await performSettingsAction(.rateApp) }
                    }
                    ...
                    settingRow(
                        ...
                        Task { await performSettingsAction(urlAction(config.termsOfUseURL, fallback: "terms_of_use")) }
                    }
                    ...
                    settingRow(
                        ...
                        Task { await performSettingsAction(urlAction(config.privacyPolicyURL, fallback: "privacy_policy")) }
                    }
                    ...
                    settingRow(
                        ...
                        Task { await performSettingsAction(feedbackAction) }
                    }
                }
    ...
    // MARK: - Actions
    private var shareAction: JetSettingsAction {
        if let url = URL(string: config.appStoreURL) {
            return .share(text: config.shareText, appStoreURL: url)
        }
        return .custom("share_app")
    }

    private var feedbackAction: JetSettingsAction {
        .feedback(email: config.feedbackEmail, subject: config.feedbackSubject)
    }

    private func urlAction(_ urlString: String, fallback: String) -> JetSettingsAction {
    ...
    @MainActor
    private func performSettingsAction(_ action: JetSettingsAction) async {
        if let handler = settingsActionHandler {
            let isRestoringPurchases = action == .restorePurchases
