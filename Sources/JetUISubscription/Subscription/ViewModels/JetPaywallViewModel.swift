...
public final class JetPaywallViewModel: ObservableObject {
    ...
    /// 可恢复购买失败状态
    @Published public private(set) var recoveryState: JetPaywallRecoveryState?
    ...
    public init(storeService: JetStoreServiceProtocol? = nil, paywallSource: String = "unknown") {
        ...
        self.storeService = storeService ?? JetSubscriptionRegistry.runtime?.storeService ?? JetStoreService()
    ...
    public func purchase(_ product: Product) async {
        ...
        recoveryState = nil
        errorMessage = nil
        AnalyticsManager.logPurchaseStart(productId: product.id, paywallSource: paywallSource)

        do {
            let (_, _) = try await storeService.purchase(product)
            let entitlementActive = await storeService.isEntitledToPro()
            ...
            shouldDismissPaywall = entitlementActive
            if !entitlementActive {
                errorMessage = SubL.Error.noActiveSubscription
                recoveryState = JetPaywallRecoveryState(category: .notEntitled)
                AnalyticsManager.logPurchaseFailure(
                    productId: product.id,
                    paywallSource: paywallSource,
            ...
            purchaseInProgress = nil
            AnalyticsManager.logPurchaseCancelled(productId: product.id, paywallSource: paywallSource)

        } catch {
            purchaseInProgress = nil
            let reasonCategory = JetPaywallFailureCategory.subscriptionCategory(for: error)
            recoveryState = JetPaywallRecoveryState(error: error)
            errorMessage = recoveryState?.message ?? error.localizedDescription
            AnalyticsManager.logPurchaseFailure(
                productId: product.id,
                paywallSource: paywallSource,
    ...
    public func restore() async {
        ...
        recoveryState = nil
        errorMessage = nil
        AnalyticsManager.logRestoreStart(paywallSource: paywallSource)
        ...
        do {
            ...
            if isPro {
                AnalyticsManager.logRestoreResult(
                    paywallSource: paywallSource,
                    result: "success",
                    reasonCategory: nil,
