...
/// Optional backend binder. Product apps or adapter modules can inject this
/// when StoreKit purchases should be synchronized to a backend.
public protocol JetSubscriptionBinder {
    func bindSubscription(signedPayload: String, storeKitType: Int, usageType: Int) async throws
}
...
public final class JetStoreService: JetStoreServiceProtocol {
    ...
    private func bindToBackend(jws: String) async throws {
        ...
        do {
            try await binder.bindSubscription(
                signedPayload: jws,
                storeKitType: 2,
                usageType: 1
            )
    ...
    public func isEntitledToPro() async -> Bool {
        ...
        do {
            ...
            AnalyticsManager.logEntitlementRefresh(
                ...
                productId: nil,
                result: "failure",
                reasonCategory: JetPaywallFailureCategory.subscriptionCategory(for: error),
                entitlementActive: false
            )
            return false
        }
    ...
    private var activeConfig: JetSubscriptionConfig? {
        config ?? JetSubscriptionRegistry.config
    }
}
