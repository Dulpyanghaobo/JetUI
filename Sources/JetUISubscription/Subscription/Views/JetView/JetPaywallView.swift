...
public struct JetPaywallView: View {
    ...
    public init(
        ...
        self.configuration = JetSubscriptionRegistry.paywallConfiguration ?? .default
        self.onSuccess = onSuccess
        self.onDismiss = onDismiss
    }
                ...
                ZStack(alignment: .top) {
                    ...
                    ScrollView(.vertical, showsIndicators: false) {
                        VStack(spacing: 24) {
                            ...
                            if let recovery = viewModel.recoveryState {
    ...
    private func recoveryStateView(_ recovery: JetPaywallRecoveryState) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            Text(recovery.title)
                .font(.headline)
                .foregroundColor(.white)
